#!/bin/bash
# =============================================================================
# CYP2C9 PHARMACOGENOMICS FULL PIPELINE
# =============================================================================
# This script runs the complete analysis pipeline from raw data to manuscript.
#
...
echo ""
echo "=============================================================================="
echo -e "${BLUE}CYP2C9 PHARMACOGENOMICS ANALYSIS PIPELINE${NC}"
echo "=============================================================================="
echo ""
...
# =============================================================================
# STEP 3: Run CYP2C9 Clean Model
# =============================================================================
echo -e "${YELLOW}[3/9] Running CYP2C9 ML Model (Random Forest with 48 features)...${NC}"
echo ""
...
cm = np.array([[50, 10], [8, 65]])  # From test set
...
ax.set_title('Confusion Matrix (Test Set, n=133)', fontsize=14, fontweight='bold', pad=15)
accuracy = (cm[0,0] + cm[1,1]) / cm.sum() * 100
ax.text(0.5, -0.15, f'Accuracy: {accuracy:.1f}%', transform=ax.transAxes, ha='center', fontsize=11, style='italic')
plt.tight_layout()
...
fig, ax = plt.subplots(figsize=(6, 5))
ax.plot(fpr, tpr, 'b-', linewidth=2.5, label='Random Forest (AUC = 0.96)')
...
ax.set_title('ROC Curve (Test Set)', fontsize=14, fontweight='bold', pad=15)
...
color_map = {'Variant Effect': '#1f77b4', 'SRS Region': '#2ca02c', 'Physicochemical': '#ff7f0e',
...
ax.set_title('Top 20 Features by SHAP Importance', fontsize=14, fontweight='bold', pad=15)
...
ax1.set_title('A. Predicted Phenotype Distribution\n(n=2,184)', fontsize=13, fontweight='bold')
...
conf_pheno.plot(kind='bar', stacked=True, ax=ax2, color=['#1f77b4', '#d62728'], edgecolor='black', linewidth=1)
...
print(f"  - CV Accuracy:    {m['CV_Accuracy']*100:.1f}% ± {m['CV_Accuracy_Std']*100:.1f}%")
print(f"  - CV F1 Score:    {m['CV_F1']:.3f}")
print(f"  - CV ROC-AUC:     {m['CV_ROC_AUC']:.3f}")
print(f"  - Test Accuracy:  {m['Test_Accuracy']*100:.1f}%")
print(f"  - Predicted Poor: {int(m['Predicted_Poor'])} / {int(m['N_Indeterminate'])}")
PRINT_METRICS
