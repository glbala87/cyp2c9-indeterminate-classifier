"""
CYP2C9 XGBOOST MODEL - WITH VALIDATED PATHOGENICITY SCORES
===========================================================
Updated with 68 validated CADD/REVEL scores from dbNSFP and CPIC.
"""
...
print("="*80)
print("CYP2C9 XGBOOST MODEL - WITH VALIDATED PATHOGENICITY SCORES")
print("="*80)
...
print(f"\nLoaded {len(VALIDATED_SCORES)} validated pathogenicity scores")
...
def get_variant_scores(position, ref_aa, alt_aa, region, grantham):
    """Get validated scores or estimate if not available"""
    key = f'p.{ref_aa}{position}{alt_aa}' if ref_aa and alt_aa and position else None

    # Use validated scores if available
    if key and key in VALIDATED_SCORES:
        return VALIDATED_SCORES[key]

    # Estimate for unknown variants (fewer now with 68 validated)
...
print(f"\n[1] Using {len(FEATURE_COLS)} features with {len(VALIDATED_SCORES)} validated scores")
...
# EXTRACT FEATURES
# ============================================================================
print("\n[3] Extracting features with validated scores...")

inter_data = []
validated_count_inter = 0
...
# ============================================================================
print("\n[4] Training XGBoost Classifier with validated scores...")
...
print(f"    Class balance - scale_pos_weight: {scale_pos_weight:.3f}")

# Use best params from previous run
best_params = {
...
cv_acc = cross_val_score(cv_model, X, y, cv=cv, scoring='accuracy')
cv_f1 = cross_val_score(cv_model, X, y, cv=cv, scoring='f1')
cv_roc = cross_val_score(cv_model, X, y, cv=cv, scoring='roc_auc')

print(f"\n{'='*70}")
print("XGBOOST MODEL WITH VALIDATED SCORES - CV RESULTS")
print(f"{'='*70}")
...
# ============================================================================
print("\n[7] Predicting Indeterminate Diplotypes...")

X_indet = indet_df[FEATURE_COLS].values
indet_pred = best_xgb.predict(X_indet)
indet_prob = best_xgb.predict_proba(X_indet)[:, 1]
...
# SAVE OUTPUTS
# ============================================================================
print(f"\n[8] Saving outputs to {OUTPUT_DIR}/...")
...
metrics_df = pd.DataFrame({
    'Metric': ['CV_Accuracy', 'CV_Accuracy_Std', 'CV_F1', 'CV_F1_Std', 'CV_ROC_AUC',
               ...
               'N_Features', 'N_Validated_Scores', 'N_Training', 'N_Indeterminate',
               'Predicted_Poor', 'Predicted_Intermediate', 'Model_Type'],
    'Value': [cv_acc.mean(), cv_acc.std(), cv_f1.mean(), cv_f1.std(), cv_roc.mean(),
              test_acc, test_f1, test_roc, test_precision, test_recall,
              len(FEATURE_COLS), len(VALIDATED_SCORES), len(train_df), len(indet_df),
...
# ============================================================================
print("\n" + "="*80)
print("XGBOOST MODEL WITH VALIDATED SCORES - SUMMARY")
print("="*80)

print(f"""
MODEL: XGBoost with {len(VALIDATED_SCORES)} Validated Pathogenicity Scores
OUTPUT: {OUTPUT_DIR}/

VALIDATED SCORES:
  - Source: dbNSFP4.3, CPIC/PharmGKB
  - Coverage: {len(VALIDATED_SCORES)} variants (vs 30 previously)
