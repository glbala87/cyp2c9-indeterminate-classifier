"""
CYP2C9 XGBOOST MODEL - Independent Features Only
=================================================
Uses XGBoost instead of Random Forest for improved performance.
Outputs to xgboost_model_output/ directory.
...
# Output directory for XGBoost model
OUTPUT_DIR = 'xgboost_model_output'
os.makedirs(OUTPUT_DIR, exist_ok=True)

print("="*80)
print("CYP2C9 XGBOOST MODEL - INDEPENDENT FEATURES ONLY")
print("="*80)
print(f"\nOutput directory: {OUTPUT_DIR}/")
print("XGBoost: Gradient Boosting with L1/L2 regularization")
...
for rank, (_, row) in enumerate(importance_df.head(15).iterrows(), 1):
    print(f"  {rank:2}. {row['Feature']:<22} SHAP: {row['SHAP_Importance']:.4f}  XGB: {row['XGB_Importance']:.4f}")

X_df = pd.DataFrame(X, columns=FEATURE_COLS)

plt.figure(figsize=(12, 14))
shap.summary_plot(shap_vals, X_df, show=False, max_display=25)
plt.title('SHAP Beeswarm - XGBoost Model', fontsize=12, pad=10)
plt.tight_layout()
...
plt.title('SHAP Feature Importance - XGBoost Model', fontsize=12, pad=10)
plt.tight_layout()
